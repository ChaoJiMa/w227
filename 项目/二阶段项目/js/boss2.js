...
//下一页
$('#next').click(function(){
    next(stu.length>0 ? stu : stuArr);
})
//上一页
$('#pre').click(function(){
    prev(stu.length>0 ? stu : stuArr);
});

//下一页函数
function next(newarr){
    var first=$('#first').text();
    var cont=$('#cont').text();
    if(first<cont){
        arr=newarr.slice(first*5,(Number(first)+1)*5);
        first++;
...
//上一页函数
function prev(newarr){
    var first=$('#first').text();
    var cont=$('#cont').text();
    if(first<=cont && first>1){
        arr=newarr.slice((first-2)*5,(first-1)*5);
        first--;
    }
    $('#first').text(first);
    render(arr);
...
$('table').on('click','.delbt',function(){
    ...
    if(stu.length==0){
        ...
        // var num1=stu[num].number;
        // var number=0;
        //判断num在原数组的下标
        for(var i=0;i<stuArr.length;i++){
            if(stuArr[i].number==stu[num].number){
                stuArr.splice(i,1);
            }
        }

        // $.each(stuArr,function(i,v){
        //     var number=v.number;
        //     if(stu[num].number==stuArr[i].number){
        //         stuArr.splice(i,1);
        //     }
        // })
...
$('table').on('click','.update',function(){
    ...
    if(stu.length==0){
        ...
        //显示信息
        $('#uname').val(stu[num].name);
        $('#age').val(stu[num].age);
        $('#id').val(stu[num].number);
        // $('#id').css('readonly',readonly);
        //判断性别
        var sex=stu[num].sex;
        if(sex=='男'){
        ...
        $('#zhuanye').val(stu[num].zhuanye);
        $('#bj').val(stu[num].bj);
        $('#city').val(stu[num].city);
    }
...
$('#tiaoZhuan').on('click',function(){
    ...
    if(index>0 && index<=cont){
        ...
        if(stu.length==0){
            ...
            arr=stu.slice((index-1)*5, index * 5);
        }
...
$('#select').on('click',function(){
    if($('#selectVal').val()==''){
        ...
        for(var i=0;i<stuArr.length;i++){
            if(stuArr[i].name.indexOf(str)>=0){
                stu.push(stuArr[i]);
                // console.log(11);
            }
        }
        ...
        //页数下标
        var index=$('#first').text();
        var stu1=stu.slice((index-1)*5, index * 5);
        // stu=stu.slice((1-1)*5, 1 * 5);
        render(stu1);
